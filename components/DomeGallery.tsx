...
type DomeGalleryProps = {
  ...
  grayscale?: boolean;
  /** Degrees per second of idle drift. 0 disables it. Dragging always wins. */
  autoRotate?: number;
};
...
export default function DomeGallery({
  ...
  // Idle drift. Yields to a drag, to a fling's inertia, and to an opened image,
  // so it never fights the viewer — it just keeps the sphere alive when nobody
  // is touching it.
  useEffect(() => {
    ...
    const step = (now: number) => {
      ...
      if (!draggingRef.current && !inertiaRAF.current && !focusedElRef.current) {
        const nextY = wrapAngleSigned(rotationRef.current.y + autoRotate * dt);
